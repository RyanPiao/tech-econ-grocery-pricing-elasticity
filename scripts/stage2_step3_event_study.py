#!/usr/bin/env python3
"""Stage 2 Step 3: event-study lead/lag table + pre-trend tests."""
...
def fit_event_study(es_df: pd.DataFrame, event_weeks: list[int]):
    ...
    formula = (
        "conversion ~ "
        + " + ".join(ev_terms)
        + " + eta_minutes + stockout_rate + rain_index + C(market_id) + C(stage) + C(hour_local)"
    )
...
def plot_event_study(table: pd.DataFrame) -> None:
    ...
    plt.scatter([-1], [0], marker="x", s=60, color="gray", label="Baseline stage (-1)")

    plt.title("Stage 2 Step 3: Event-Study Lead/Lag Estimates (Conversion)")
    plt.xlabel("Event stage relative to rollout")
